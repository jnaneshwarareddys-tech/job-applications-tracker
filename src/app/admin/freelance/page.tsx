...
export default function FreelanceAdminDashboard() {
  const { jobs, loading, addJob, updateJob, deleteJob } = useJobs('freelance_applications');
  ...
  const handleDelete = (id: string) => {
    if (!window.confirm('Are you sure you want to delete this freelance gig?')) return;
    deleteJob(id);
  };
      ...
      <main className={styles.main} style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '80vh' }}>
        <div className="glass" style={{ padding: '2rem', borderRadius: '12px', width: '100%', maxWidth: '400px', textAlign: 'center' }}>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <Link href="/freelance" style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', marginTop: '1rem', textDecoration: 'underline' }}>
              Return to Freelance Dashboard
            </Link>
          </form>
        </div>
      </main>
  ...
  return (
    <main className={styles.main}>
      <header className={styles.header}>
        <div>
          <h1 className={styles.title}>Freelance Admin</h1>
          <p className={styles.subtitle}>Manage your thumbnail gigs below.</p>
        </div>
        <Link href="/freelance" className={styles.navLink}>
          Back to Dashboard
        </Link>
      </header>
